...
class AdiabaticCorrectionPreProcessor(Processor):
    def __init__(self, context: Context, **kwargs):
        ...
        correct_elevation = context.checkpoint.supporting_arrays[
            "lam_0/correct_elevation"
        ]

        self._corrector = AdiabaticCorrector(
            model_elevation * units.meters, correct_elevation * units.meters
        )
...
class AdiabaticCorrector:
    def __init__(
        self, model_elevation: pint.Quantity, correct_elevation: pint.Quantity
    ):
    ...
    def apply(self, fields: ekd.FieldList) -> ekd.FieldList:
        ...
        for t in temperatures:
            ...
            corrected_temperature = adiabatic_correct.correct_temperature(
                values, self._altitude_difference
            )
        ...
        for field in fields:
            ...
            if param == "2t":
                ...
                values = adiabatic_correct.correct_surface_pressure(
                    old_values, self._altitude_difference
                )  # type: ignore
                ret.append(field.copy(values=values.magnitude))  # type: ignore
            elif param == "z":
                values = adiabatic_correct.convert_to_geopotential(
                    self._correct_elevation
                )
                ret.append(field.copy(values=values.magnitude))  # type: ignore
            else:
                ret.append(field)
