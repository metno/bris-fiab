...
@click.option(
    ...
    type=str,
    required=True,
    help="New area in the format north/west/south/east.",
)
@click.option(
    "--add-fiab-metadata",
    is_flag=True,
    default=False,
    help="Add forecast-in-a-box metadata to the checkpoint.",
)
@click.option(
    "--create-sample-config",
    is_flag=True,
    default=False,
    help="Create a sample config file for the new domain for use with anemoi inference. It will be named as <dest>.yaml.",
)
@click.option(
    "--global-grid",
    type=str,
    default="n320",
    show_default=True,
    help="Global grid to use, e.g. n320.",
)
...
@click.option(
    ...
    default=None,
    help="Path to a local orography file (GeoTIFF). If not provided, the script will download orography data from OpenTopography.org.",
)
@click.option(
    "--save-graph-to",
    type=click.Path(),
    default=None,
    help="If provided, saves the generated graph to the specified path for reuse.",
)
...
def move_domain(
    grid: float,
    area: str,
    add_fiab_metadata: bool,
    create_sample_config: bool,
    global_grid: str,
    lam_resolution: int,
    global_resolution: int,
    margin_radius_km: int,
    orography_file: str | None,
    save_graph_to: str | None,
    src: str,
    dest: str,
) -> None:
...
def get_orography_stream(
    orography_file: str | None, north: str, west: str, south: str, east: str
) -> io.BufferedIOBase:
