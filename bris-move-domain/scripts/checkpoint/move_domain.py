import click
from bris_move_domain.orography import download, api_key
from bris_move_domain.checkpoint import graph
import tempfile
import yaml
import os
import io
...
def move_domain(grid: float, area: str, add_fiab_metadata: bool, create_sample_config: bool, global_grid: str, lam_resolution: int, global_resolution: int, margin_radius_km: int, orography_file: str | None, src: str, dest: str) -> None:
    ...
    if add_fiab_metadata:
        from bris_move_domain.checkpoint.fiab import add_fiab_metadata_to_checkpoint
        add_fiab_metadata_to_checkpoint(grid, area, global_grid, dest)

    if create_sample_config:
        from bris_move_domain.checkpoint.config import save_sample_config
